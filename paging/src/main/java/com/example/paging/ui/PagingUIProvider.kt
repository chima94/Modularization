...
class PagingUIProvider @Inject constructor(
    ...
    override fun onPaginationReachedError(append: LoadState, errorMessage: Int) {
    ...
    override fun <T : Any> OnError(
        ...
        if(isLoadStateNoConnectionException(refresh) || isLoadStateNoConnectionException(append) || isLoadStateNoConnectionException(prepend)){
            if(pagingItems.itemCount == 0){
                ...
                toaster.longToast(com.example.strings.R.string.no_connection_message_will_load_on_connection_achieved)
            }
